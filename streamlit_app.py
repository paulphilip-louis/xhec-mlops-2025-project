...
def main():
    ...
    if not api_healthy:
        st.error("❌ L'API FastAPI n'est pas accessible. Assurez-vous qu'elle est démarrée sur http://localhost:8000")
        st.stop()
    else:
    ...
    sex = st.sidebar.selectbox(
        "Sexe",
        options=["M", "F", "I"],
        help="M = Mâle, F = Femelle, I = Immature"
    )
    ...
    if st.sidebar.button("🔮 Prédire l'Âge", type="primary"):
        ...
        if prediction:
            ...
            with col2:
                st.metric(
                    label="Confiance",
                    value=f"{confidence:.2%}" if isinstance(confidence, (int, float)) else "N/A",
